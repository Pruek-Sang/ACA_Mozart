        ...
        try {
            ...
            const excelData = [
                ...
                ...loads.map((item: LoadResult, idx: number) => [
                    idx + 1,
                    item.device_name || '-',
                    item.load_va_l1 || Math.round(item.power_kw * 1000) || 0,
                    item.load_va_l2 || 0,
                    item.load_va_l3 || 0,
                    item.breaker_type || 'MCB',
                    `${item.breaker_poles || 1}P`,
                    `${item.breaker_ic_ka || 6}kA`,
                    item.breaker_af || item.breaker_size || 15,
                    item.breaker_at || item.breaker_size || 15,
                    item.wire_size_l || item.wire_size?.replace(' mm²', '') || '2.5',
                    item.wire_size_n || item.wire_size?.replace(' mm²', '') || '2.5',
                    item.wire_size_grd || item.ground_size || '2.5',
                    item.wire_type || 'THW',
                    item.conduit_size || '1/2"',
                    item.conduit_type || 'PVC',
                ...
                [],
                // Summary rows
                // Summary rows (Using values from data.data or fallback calculation)
                ['TOTAL LOAD (VA)', '', loads.reduce((sum: number, item: LoadResult) => sum + (item.load_va_l1 || item.power_kw * 1000 || 0), 0), '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''],
